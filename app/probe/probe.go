...
func handleOne(ctx context.Context, pool dcpool.Pool, manager *peers.Manager, peer tg.InputPeerClass, msgID int, opts Options) error {
	...
	hashHex, downloaded, err := hashPartialSmart(ctx, client, media.InputFileLoc, start, want, opts.Algo)
	if err != nil {
		// Fallback to aligned mode if precise request failed (some servers may not support Precise).
		if fh, fd, ferr := hashPartialAligned(ctx, client, media.InputFileLoc, start, want, opts.Algo); ferr == nil {
			hashHex, downloaded = fh, fd
			err = nil
		}
	}
	if err != nil {
...
func hashPartialSmart(ctx context.Context, api *tg.Client, loc tg.InputFileLocationClass, start, length int64, algo string) (string, int64, error) {
...
func hashPartialMiBAligned(ctx context.Context, api *tg.Client, loc tg.InputFileLocationClass, start, length int64, algo string) (string, int64, error) {
	var downloaded int64
	...
	for offset < end {
		...
		if err != nil {
			return "", downloaded, errors.Wrap(err, "UploadGetFile (1MiB aligned)")
		}
		...
		case *tg.UploadFile:
			...
			if maxWrite > 0 {
				...
				if _, err := hasher.Write(chunk); err != nil {
					return "", downloaded, errors.Wrap(err, "hash write (1MiB aligned)")
				}
...
func hashPartialAligned(ctx context.Context, api *tg.Client, loc tg.InputFileLocationClass, start, length int64, algo string) (string, int64, error) {
	var downloaded int64
	...
	for offset < end {
		...
		if err != nil {
			return "", downloaded, errors.Wrap(err, "UploadGetFile (aligned)")
		}
		...
		case *tg.UploadFile:
			...
			if maxWrite > 0 {
				chunk := data[begin : begin+maxWrite]
				if _, err := hasher.Write(chunk); err != nil {
					return "", downloaded, errors.Wrap(err, "hash write (aligned)")
				}
				downloaded += int64(len(chunk))
			}
			offset += int64(len(data))
